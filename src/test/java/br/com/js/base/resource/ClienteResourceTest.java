...
import br.com.js.base.helper.EnderecoTestHelper;
...
//		Mockito.doNothing().when(service).delete(Mockito.anyLong());
  ...
  public void deve_pesquisar_uma_lista_cliente_por_nome() throws Exception {
    ...
    // Execução
    var request = MockMvcRequestBuilders.get(URL_API + "?nome=teste").header("Authorization", "Bearer " + accessToken)
  ...
  public void deve_retornar_not_found_ao_buscar_um_cliente_pelo_codigo_invalido() throws Exception {
    // @formatter:off
  ...
  public void deve_pesquisar_enderecos_do_cliente() throws Exception {
    var cliente = ClienteTestHelper.getCliente(1l);
    cliente.setEnderecos(EnderecoTestHelper.getAddressList());
